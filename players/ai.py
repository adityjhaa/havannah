...
class Node:
    ...
    def best_child(self, c=1.41, beta_func=None):
    ...
    def update_rave(self, move, outcome):
...
# MCTS function remains the same but integrates RAVE statistics
def mcts(state, timer_per_move, player_number, target_depth=3):
    ...
    while time.time() - start_time < timer_per_move and not max_depth_reached:
        leaf_node, depth = tree_policy(root, player_number, current_depth=0)
        if leaf_node is None:
            continue
        outcome = rollout(leaf_node, player_number)
        backpropagate(leaf_node, outcome)
        
        # Update RAVE statistics for the current rollout
        node = leaf_node
        while node.parent:
    ...
    return root.best_child(c=1.41).move
